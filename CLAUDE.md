...
10. **Vertical slices, not big-bang builds.** Implementation follows a fixed slice order: Infrastructure → Auth → Core Feature → Additional Features. Each slice is a thin vertical cut through the full stack (DB migration + backend endpoints + frontend pages) that can be independently validated. Within a slice, backend + frontend + db run in parallel. Between slices, the orchestrator runs a validation gate (Playwright browser test) before starting the next slice. Bugs caught after a 2-file slice diff are cheap. Bugs caught after the entire codebase is assembled are expensive.
11. **Full integration tests gate every build — not just curl.** After `docker compose up --build` + `seed.py`, two test suites must pass before the build is declared complete:
