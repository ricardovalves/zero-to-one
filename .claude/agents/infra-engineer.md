...
- **`docker compose up --build` must be run and the stack must reach healthy state before declaring the build complete.** A build that compiles but fails to start is not a deliverable. After `docker compose up --build`, verify: (1) all containers reach healthy/running state, (2) `GET /health` returns 200, (3) seeded users can log in via the dev panel and land on a populated dashboard — not a blank screen or error. If you cannot run docker compose yourself (agent environment limitation), flag this explicitly to the orchestrator rather than declaring the build complete.
- **`docker compose build` must also be verified (build step alone) before declaring the build complete.** A Dockerfile that fails to build is not a deliverable. Common failure modes to check: missing source directories referenced in `COPY` (e.g. `public/` in Next.js), packages in `requirements.txt` that don't belong in the Docker image (CI tools, security scanners), and `COPY` paths that assume files exist but were never created by the application engineer.
...
- **PostgreSQL healthcheck must verify the role, not just TCP connectivity.** `pg_isready` returns exit 0 even when the user/role doesn't exist — it only checks that the server accepts connections. Use `PGPASSWORD=<pass> psql -U <user> -d <db> -c 'SELECT 1' -q 2>/dev/null` instead. This catches the common failure where a stale Docker volume from a prior run (with a different POSTGRES_USER) causes "role does not exist" errors because PostgreSQL ignores init env vars on non-empty data directories.
- **Env var names in docker-compose must match the field names in pydantic-settings config.** If `config.py` reads `SMTP_HOST`, the docker-compose environment block must set `SMTP_HOST:`, not `MAIL_HOST:`. Pydantic-settings with `extra="ignore"` silently drops mismatched names. Always verify env var names against the Settings class before writing docker-compose.
- **Do not block backend startup on LiteLLM being healthy.** LiteLLM proxy can take 30+ seconds to start and its `/health` endpoint may return errors when no API keys are pre-configured (keys are per-user at runtime). Use `condition: service_started` (not `service_healthy`) for LiteLLM in the backend's `depends_on`. The backend only needs LiteLLM when a user actually triggers an import — not at startup. Use `python3 -c "import urllib.request; urllib.request.urlopen('http://localhost:4000/health', timeout=5)"` as the healthcheck (curl may not be present in the LiteLLM image).
