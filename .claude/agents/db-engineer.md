...
- **`psycopg2-binary` is required alongside `asyncpg`** when using Alembic with an async SQLAlchemy setup. The env.py strips `+asyncpg` from `DATABASE_URL` to produce a synchronous URL for Alembic's migration runner — that sync URL requires psycopg2, not asyncpg. Always include `psycopg2-binary>=2.9.9` in `requirements.txt` alongside `asyncpg`. Without it, `alembic upgrade head` fails with `ModuleNotFoundError: No module named 'psycopg2'` at container startup.
- **UUID migration columns must use `postgresql.UUID`, not `sa.String(36)`** — if the Alembic migration creates UUID PK columns as `sa.String(36)` (VARCHAR) but the ORM model uses `UUID(as_uuid=True)`, asyncpg raises `"operator does not exist: character varying = uuid"` on any `session.refresh()` or primary-key lookup. Always use `from sqlalchemy.dialects.postgresql import UUID as PG_UUID` in migrations and declare UUID columns as `PG_UUID(as_uuid=True)` with `server_default=sa.text("gen_random_uuid()")` (no `::text` cast). The ORM model must use `UUID(as_uuid=True)` to match.
